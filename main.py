...
def evaluate(model, valid_X, attack_path, output_file):
    if "OneClassSVM" in str(type(model)):
        ...
        threshold = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9,
                     0.95]
...
def read_kdd_dataset(path):
    global label_encoder_1, label_encoder_2, label_encoder_3
    ...
    dataset[:, 1] = label_encoder_1.fit_transform(dataset[:, 1]) if not hasattr(label_encoder_1,
                                                                                'classes_') else label_encoder_1.transform(
        dataset[:, 1])
    dataset[:, 2] = label_encoder_2.fit_transform(dataset[:, 2]) if not hasattr(label_encoder_2,
                                                                                'classes_') else label_encoder_2.transform(
        dataset[:, 2])
    dataset[:, 3] = label_encoder_3.fit_transform(dataset[:, 3]) if not hasattr(label_encoder_3,
                                                                                'classes_') else label_encoder_3.transform(
        dataset[:, 3])

    # Define columnas categóricas y numéricas
    categorical_indices = [1, 2, 3]
    num_indices = [i for i in range(dataset.shape[1] - 2) if i not in categorical_indices]

    # Crea un ColumnTransformer que aplica OneHotEncoder solo a las categóricas
    preprocessor = ColumnTransformer(
        transformers=[
            ('cat', preprocessing.OneHotEncoder(handle_unknown='ignore', sparse=False), categorical_indices),
            ('num', 'passthrough', num_indices)
        ]
    )

    # Aplica el preprocesamiento
    dataset_features = preprocessor.fit_transform(dataset[:, :-2])
...
if __name__ == '__main__':
    ...
    if args.model == 'svm':
        ...
        reconstructions = wrapper.model.predict(valid_X)
        mse = np.mean(np.square(valid_X - reconstructions), axis=1)

        # Etiquetas verdaderas (0 = normal, 1 = anomalía)
        # Como estamos usando solo tráfico normal, podemos simular etiquetas:
        y_true = np.zeros_like(mse)

        # Puedes establecer un umbral personalizado o usar wrapper.mse_threshold
        threshold = wrapper.mse_threshold
        y_pred = (mse > threshold).astype(int)

        # Ahora imprimimos las métricas (se verán todas en 0 porque valid_X es solo normal)
